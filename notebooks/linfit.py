def LineFitWt(x, y, dy):
    ...
    >>>LineFitWt(2.,3.,1)
    *** Test This Part!!!!****
    cant seem to run code to get an output...
    ...
    def test_LineFitWt():
        out = LineFitWt(np.linspace(1,5,5),np.linspace(1,8,5),\
         np.linspace(0.2,0.8,5)])
        ntest.assert_almost_equal(out, [slope, yint, sigma_a, sigma_b])

    if __name__ == "__main__":

        print('running LineFitWt test')
        test_LineFitWt()
        doctest.testmod()
